...
export default function FormCounterparty({
  ...
  const { dataGeo } = useGeo(
    ID_USER,
    PURPOSE_USE.redact,
    "добавил нового контрагента",
  );
  ...
  const addNewCounterParty = async () => {
    setLoader(true);
    const dataNewCounterparty = await FetchCounterparty.createCounterparty(
      INN,
      { phone: phoneValue },
      dataGeo,
    );
              ...
              <CusConfigProvider>
                <ul>
                  <li className="flex gap-1 items-center mb-2">
                    ...
                    <p className="text-xs">
                      {format(new Date(formData.dateCreate), "dd/mm/yy")}
                    </p>
                  </li>
                  ...
                  <li className=" mt-2">
                    <Link
                      href={`${pathname}/payment/counterparty/${formData._id}`}
                    >
                      <p>подробнее...</p>
                    </Link>
                  </li>
