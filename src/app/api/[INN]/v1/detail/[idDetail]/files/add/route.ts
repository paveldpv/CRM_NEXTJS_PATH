...
export async function PUT(request: NextRequest, { params }: { params: { INN: string; idDetail: string } }) {
	...
	const { dataFiles, dataGeo } = body as {
		dataFiles: TResponseUploadFiles[]
		dataGeo: TNewDataGeoLocationDTO
	}

	const ids = MongoHelpers.stringsToObjectIdsTuple(idDetail, dataGeo.user)

	if (ids === null) {
		return NextResponse.json({ message: 'Invalid ID format' }, { status: 400 })
	}
	const [detailId, userId] = ids
   ...
   const result = await Promise.all([
    serviceDetail.addFilesFromDetail(detailId, dataFiles),
