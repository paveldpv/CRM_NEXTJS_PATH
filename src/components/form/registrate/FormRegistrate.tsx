...
export default function FormRegistrate() {
	...
	const [loader, setLoader] = useMiniLoader((state) => [
		state.visible,
	...
	const onSubmit = async () => {
		...
		navigator.geolocation.getCurrentPosition(
			async (pos) => {
				...
				if (
					candidateNewAdmin.status === 200 &&
					candidateNewAdmin.response === 'OK'
				) {
					...
					setOpenDialog(
						true,
						{
							...
							message: `статус сервера : ${
								...
								candidateNewAdmin.response !== 'OK' &&
								candidateNewAdmin.response.message
							}`,
						},
						typeDialog.error
					)
			...
			>
				{fieldData.map((field, index) => (
					<TextField
							...
							startAdornment: (
								<InputAdornment position='start'>
									<span
										...
										onClick={() =>
											field.name === 'password' && dispatchVisiblePas()
										}
									>
										{field.name === 'password' ? (
						...
						helperText={
							<span className=' text-red-800'>{errors[field.title]}</span>
						}
