...
import { TPreviewUploadFile } from '../model/type'
...
export default function PreviewPictureFile({
	...
	deletedFile,
	...props
}: Omit<TPreviewUploadFile, 'preview'> & TResponseUploadFiles & DetailedHTMLProps<HTMLAttributes<HTMLDivElement>, HTMLDivElement>) {
	const { FullPath } = props
	return (
		<div className={cn('flex justify-center items-center relative', props.className)}>
			<Image height={height} width={width} src={`${SERVER_DOTNET}/${FullPath}`} alt={alt || NOT_FOUND_PICTURE} className=' rounded-md ' />
