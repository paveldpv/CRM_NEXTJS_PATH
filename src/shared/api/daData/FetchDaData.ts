...
export class FetchDaData {
	static async updateDaData(
		...
		const updateDaData = await serverClient.api<TDaDataOrganizationDTO>(INN,`${INN}/daData/update`, {
			method: 'POST',
	...
	static async getDaDataByINN(INN: string, queryINN: string): Promise<TDaDataOrganizationDTO> {
		const getDataData = await serverClient.api<TDaDataOrganizationDTO>(INN,`${INN}/daData/get/INN`, {
			method: 'POST',
	...
	static async getAllDaData(INN: string): Promise<TDaDataOrganizationDTO[]> {
		const getAllDaData = await serverClient.api<TDaDataOrganizationDTO[]>(INN,`${INN}/daData/get/all`, { method: 'GET' })
		return getAllDaData
	}
	static async getAllDaDataWithDeleted(INN: string): Promise<TDaDataOrganizationDTO[]> {
		const getAllDaDataWithDeleted = await serverClient.api<TDaDataOrganizationDTO[]>(INN,`${INN}/daData/get/withDeleted`, {
			method: 'GET',
		})
