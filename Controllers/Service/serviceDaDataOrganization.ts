...
export class ServiceDaDataOrganization extends Service {
	...
	public async addDaData(
		query: TQueryGetDaDataOrganization
	): Promise<TDaDataOrganization | TError> {
		try {
        ...
        this.logError({error,message})
			...
			await new ControllerDaDataOrganizationDB(this.INN).addDaData(
				daDataOrganization
			)
  ...
  public async getDaDataByINN(INN:string):Promise<TDaDataOrganization|null|TError>{
    try {
      ...
      const err:TError={
        error:true,message:`error get daData by INN inn ${INN}, error ${error}`
      }
      this.logError(err)
      return err
    }
  ...
  public async getAllDaData():Promise<TDaDataOrganization[]|[]|TError>{
    try {
      ...
      const err:TError={
        error:true,message:`error get all  da data ,error ${error}`
      }
      this.logError(err)
      return err
    }
  ...
  public async deletedDaDataByINN(INN:string):Promise<void|TError>{
    try {
      ...
      const err:TError={
        error:true,message:`error deleted da data by INN INN:${INN},error :${error}`
      }
    }
  }
  public async updateDaDataByINN(newDaData:TDaDataOrganization):Promise<void|TError>{
    try {
      ...
      const err:TError={
        error:true,message:`error update da data by INN ,new data :${newDaData},error ${error}`
      }
      this.logError(err)
      return err
    }
  ...
  public async getDaDataRuleOrganization():Promise<TDaDataOrganization|null|TError>{
    try {
      ...
      const err:TError={
        error:true,message:`error get da data current organization error : ${error}`
      }
      this.logError(err)
      return err
    }
