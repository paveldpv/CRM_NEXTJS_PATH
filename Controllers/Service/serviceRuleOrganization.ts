...
import { getAbbreviated } from '../../function/getAbbreviated'
...
export class ServiceRuleOrganization extends Service {
	...
	public async getNameOrganization():Promise<TNameOrganization|TError>{
		try {
			...
			if(nameOrganization==null){
				const er:TError={
					error:true,message:`organization not found , inn:${this.INN}`					
				}
				this.logError(er)
				return er
			}
			...
			const er:TError={
				error:true,message:`error get name organization,error ${error}`
			}
			this.logError(er)
			return er
		}
	...
	public async getParamsOrganization(): Promise<TDataOrganization | TError> {
		try {
			const dataOrganization = await new ControllerRuleOrganizationDB(
				this.INN
			).getInfoOrganization()
	...
	public async updateParamsOrganization(
		data: TDataOrganization
	): Promise<void | TError> {
		try {
			await new ControllerRuleOrganizationDB(this.INN).updateInfoOrganization(
				data
			)
		} catch (error) {
			const err: TError = {
	...
	public async createNewRuleOrganization(
		dataGeo: TGeoLocation
	): Promise<void | TError> {
		
		const daDataOrganization = await new ServiceDaDataOrganization(
			this.INN
		).addDaData({ query: this.INN })
		...
		const initialDataOrganization = this.getInitialDataOrganization(
			daDataOrganization,
			currentDate
		)

		const saveGeoLocation = new ServiceGeoLocation(this.INN).setDataLocation(
			dataGeo
		)

		const saveInitialDataOrganization = new ControllerRuleOrganizationDB(
			this.INN
		).addInfoOrganization(initialDataOrganization)

		const saveRequisites = new ServiceRequisites(this.INN).addNewRequisites(
			daDataOrganization
		)
