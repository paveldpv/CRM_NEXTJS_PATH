...
export class ServiceConfigApp extends Service {
	...
	public async addNewPersonalConfig(idUser: string):Promise<void|TError> {
		try {
			...
			await new ControllerConfigDB(this.INN).addNewPersonalConfigApp(
				initialConfigApp
			)
		} catch (error) {
			const err:TError={
				error:true,message:`error add new personal config app, error ${error}`
			}
			this.logError(err)
			return err
		}
	...
	public async getPersonalConfig(idUser: string):Promise<null|TConfigAPP|TError> {
		try {
			...
			const er:TError = {
				error :true,message:`error get personal config app , id user :${idUser},error :${error}`
			}
			this.logError(er)
			return er
		}
	...
	public async updatePersonalConfig(newDataConfig: TConfigAPP):Promise<void|TError> {
		try {
			...
			const er :TError ={
				error:true,message:`error update personal config app ,new Data config app  :${newDataConfig}, error ${error}`
			}
			this.logError(er)
			return er
		}
